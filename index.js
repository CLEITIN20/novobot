...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `iae membro novo @${num.split('@')[0]}\seja bem vendo a esse maravilhoso grupo *${mdata.subject}* `
				...
				teks = `finalmente mais um corno saiu do grupo🥳 @${num.split('@')[0]} relaxa ele saiu para falar da vida dos outros`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
          ...
          if (!isRegistered) return reply(ind.noregis())
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'no máximo 10 caracteres', text, {quoted: mek})
				...
				case 'bc': 
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('ícone alterado com sucesso')
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('quem você deseja adiciona no grupo?')
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(` @${mentioned[0].split('@')[0]} virou membro um comum🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `parabéns🥳 Agora você não e mais um membro comum (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`parabéns🥳 @${mentioned[0].split('@')[0]} ??𝗻𝗱𝗮 ??𝗮𝗶𝗸 𝗺????𝗷??𝗱𝗶 𝗮??𝗺𝗶𝗻 𝗴𝗿𝗼𝘂?? (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `ele mereceu ser expulso 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`mais um otário foi expulso @${mentioned[0].split('@')[0]} 🏃`, bye, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou com sucesso o recurso de boas-vindas à esquerda neste grupo️')
					} else if (Number(args[0]) === 0) {
				...
				case 'clone':
					...
					if (args.length < 1) return reply(' *TAG QUE QUERIA NO CLONE!* ')
